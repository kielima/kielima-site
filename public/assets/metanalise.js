...
(function () {
  ...
  /* Contagem viva das 4 etapas da triagem (articles_final.xlsx, coluna
     ...
     Aceito da anterior. e4.principal + e4.sensibilidade = e4.n = os 533
  ...
  var FUNNEL = [
    ...
    { key: 'e2', n: 605 },
    { key: 'e3', n: 590 },
    { key: 'e4', n: 533, principal: 422, sensibilidade: 111 }
  ];
  ...
  var COPY = {
    PT: {
      ...
      funnelLabel: 'TRIAGEM',
      funnelTitle: 'O funil da triagem: de 5.093 registos a 533 artigos',
      ...
      funnelDetails: {
        universo: '5.093 registos recolhidos nas buscas. 2.627 (51,6%) eram duplicados, removidos antes de qualquer triagem. 1.865 foram rejeitados ao longo das quatro etapas seguintes — ver o detalhe em cada barra abaixo. 7 nunca tiveram o texto completo disponível (sem acesso). Dos 594 restantes, 590 — 533 elegíveis + 57 revisões sem ACV própria — chegam à Etapa 4; 4 elegíveis aceites numa revisão pontual ainda aguardam as Etapas 3 e 4.',
        ...
        e3: 'Dos 597 artigos avaliados quanto à restrição geográfica do material, 590 foram aceites; 7 foram excluídos por o material principal ser geograficamente restrito (ex.: resíduo de produção regional concentrada).',
        e4: 'Dos 590 artigos avaliados na Etapa 4 (checklist de 7 itens, 0 a 7 pontos): 422 formam o resultado principal (score ≥ 4,0), 111 entram só na análise de sensibilidade (score < 4,0), e 57 são revisões sem avaliação de ciclo de vida própria — não pontuam para o indicador ci, mas alimentam o ramo de citação do PRISMA (fontes primárias descobertas por essas revisões).'
      },
    ...
    EN: {
      ...
      funnelTitle: 'The screening funnel: from 5,093 records to 533 articles',
      ...
      funnelDetails: {
        universo: "5,093 records collected in the searches. 2,627 (51.6%) were duplicates, removed before any screening. 1,865 were rejected across the four stages that follow — see the detail in each bar below. 7 never had the full text available (no access). Of the remaining 594, 590 — 533 eligible + 57 reviews without their own LCA — reach Stage 4; 4 eligible articles accepted in a one-off review still await Stages 3 and 4.",
        ...
        e3: "Of the 597 articles assessed for the material's geographic restriction, 590 were accepted; 7 were excluded because the main material is geographically restricted (e.g. a regionally concentrated production by-product).",
        e4: "Of the 590 articles assessed in Stage 4 (a 7-item checklist, 0 to 7 points): 422 form the main result (score ≥ 4.0), 111 enter only the sensitivity analysis (score < 4.0), and 57 are reviews without their own life-cycle assessment — they don't score toward the ci indicator, but feed the PRISMA citation branch (primary sources discovered through those reviews)."
      },
    ...
    ZH: {
      ...
      funnelLabel: '筛选',
      funnelTitle: '筛选漏斗：从5,093条记录到533篇文献',
      ...
      funnelDetails: {
        universo: '检索共收集5,093条记录。其中2,627条（51.6%）为重复记录，在任何筛选之前已被移除。1,865条在随后四个阶段中被剔除——详见下方各阶段柱状图。7条从未能获取全文（无法访问）。剩余594条中，590条——533篇入选文献+57篇无自身生命周期评估的综述——进入第4阶段；另有4篇在一次专项复核中入选的文献尚待第3、4阶段评估。',
        ...
        e3: '在评估材料地域限制的597篇文献中，590篇被接受；7篇因主要材料受地域限制（例如某地区高度集中的生产副产品）而被排除。',
        e4: '在第4阶段（7项清单，0至7分）评估的590篇文献中：422篇构成主要结果（得分≥4.0），111篇仅用于敏感性分析（得分<4.0），57篇是没有自身生命周期评估的综述——不计入ci指标得分，但作为PRISMA引文分支的来源（这些综述引用的原始研究）。'
      },
      tierLabel: '质量分层',
  ...
  var KPI_VALUES = {
    ...
    /* % de artigos Elegível com pelo menos um par fck+CO2 completo em
       ...
       isentas, 458 dos 535 com par completo = 86%. funil = 533 Elegível que
       passaram as 4 etapas (2687, 3001, 3163 e 4931, aceites a 2026-09-14,
       ainda sem E3/E4). vida = artigos Elegível com vida_util_anos em
       dados_fck.
