...
(function () {
  ...
  var PROGRESS_FCK_CO2 = [
    ...
    { v: 1200, kind: 'wave' },
    { v: 1208, kind: 'wave' },
    { v: 1169, kind: 'marco', tKey: 'marcoRejeitados' }
  ];
  ...
  /* Contagem viva das 4 etapas da triagem (articles_final.xlsx, coluna
     ...
     Aceito da anterior. e4.principal + e4.sensibilidade = e4.n = os 531
  ...
  var FUNNEL = [
    ...
    { key: 'e2', n: 605 },
    { key: 'e3', n: 588 },
    { key: 'e4', n: 531, principal: 422, sensibilidade: 109 }
  ];
  ...
  var COPY = {
    PT: {
      ...
      funnelLabel: 'TRIAGEM',
      funnelTitle: 'O funil da triagem: de 5.093 registos a 531 artigos',
      ...
      funnelDetails: {
        universo: '5.093 registos recolhidos nas buscas. 2.627 (51,6%) eram duplicados, removidos antes de qualquer triagem. 1.869 foram rejeitados ao longo das quatro etapas seguintes — ver o detalhe em cada barra abaixo. 7 nunca tiveram o texto completo disponível (sem acesso). Dos 590 restantes, 588 — 531 elegíveis + 57 revisões sem ACV própria — chegam à Etapa 4; 2 elegíveis aceites numa revisão pontual ainda aguardam as Etapas 3 e 4.',
        ...
        e2: 'Dos 2.188 artigos com PDF lido na íntegra, 605 foram aceites. Dos 1.583 excluídos: 1.297 não relatavam resistência à compressão própria pareável a um CO₂ próprio, 388 não relatavam CO₂/ACV com resultado absoluto — alguns falham nos dois critérios.',
        e3: 'Dos 599 artigos avaliados quanto à restrição geográfica do material, 588 foram aceites; 11 foram excluídos por o material principal ser geograficamente restrito (ex.: resíduo de produção regional concentrada).',
        e4: 'Dos 588 artigos avaliados na Etapa 4 (checklist de 7 itens, 0 a 7 pontos): 422 formam o resultado principal (score ≥ 4,0), 109 entram só na análise de sensibilidade (score < 4,0), e 57 são revisões sem avaliação de ciclo de vida própria — não pontuam para o indicador ci, mas alimentam o ramo de citação do PRISMA (fontes primárias descobertas por essas revisões).'
      },
      ...
      progressCorpoProvaP: 'Contagem de linhas com o volume do corpo de prova (ou do elemento medido) já extraído — necessário para a futura conversão cubo→cilindro. Rastreamento começou em 12.09.2026; a extração dedicada (437 artigos, 30 lotes) terminou em 24.09.2026.',
      progressIcTitle: 'Intervalo de confiança por estudo',
      progressIcP: 'Contagem de linhas com desvio-padrão, coeficiente de variação ou intervalo de confiança do próprio artigo já extraído — base do critério QG da Etapa 4 (ver "Sobre o indicador" abaixo). Coluna criada em 12.09.2026; QG já pontuado em todos os artigos avaliados na Etapa 4.',
      progressWave: 'onda',
      progressWaveSuffix: '',
      ...
      progressMarcos: {
        ...
        marcoRejeitados: '<b>Correção · 26.09.2026</b><br>O painel passou a incluir só artigos Elegível. Até aqui, 34 pares de 6 artigos rejeitados a 21.09 pela regra E2 mais restritiva continuavam no gráfico, e 4 artigos de rejeitos de mineração foram rejeitados hoje na Etapa 3 (regionalização) — mais 5 pares.<br><b>1.208 → 1.169</b> pares.'
      },
    ...
    EN: {
      ...
      funnelTitle: 'The screening funnel: from 5,093 records to 531 articles',
      ...
      funnelDetails: {
        universo: "5,093 records collected in the searches. 2,627 (51.6%) were duplicates, removed before any screening. 1,869 were rejected across the four stages that follow — see the detail in each bar below. 7 never had the full text available (no access). Of the remaining 590, 588 — 531 eligible + 57 reviews without their own LCA — reach Stage 4; 2 eligible articles accepted in a one-off review still await Stages 3 and 4.",
        ...
        e2: 'Of the 2,188 articles fully read as PDF, 605 were accepted. Of the 1,583 excluded: 1,297 did not report their own compressive strength paired with their own CO₂, 388 did not report CO₂/LCA with an absolute result — some fail both criteria.',
        e3: "Of the 599 articles assessed for the material's geographic restriction, 588 were accepted; 11 were excluded because the main material is geographically restricted (e.g. a regionally concentrated production by-product).",
        e4: "Of the 588 articles assessed in Stage 4 (a 7-item checklist, 0 to 7 points): 422 form the main result (score ≥ 4.0), 109 enter only the sensitivity analysis (score < 4.0), and 57 are reviews without their own life-cycle assessment — they don't score toward the ci indicator, but feed the PRISMA citation branch (primary sources discovered through those reviews)."
      },
      ...
      progressCorpoProvaP: 'Count of rows with the specimen (or measured element) volume already extracted — needed for the planned cube-to-cylinder conversion. Tracking started on 09.12.2026; the dedicated extraction (437 articles, 30 batches) finished on 09.24.2026.',
      progressIcTitle: 'Confidence interval per study',
      progressIcP: 'Count of rows with a standard deviation, coefficient of variation, or confidence interval already extracted from the article itself — the basis of the Stage 4 QG criterion (see "About the indicator" below). Column created on 09.12.2026; QG is now scored for every article assessed in Stage 4.',
      progressWave: 'wave',
      progressWaveSuffix: '',
      progressStart: 'start',
      ...
      progressMarcos: {
        ...
        marcoRejeitados: '<b>Correction · 09.26.2026</b><br>The dashboard now includes only Eligible articles. Until now, 34 pairs from 6 articles rejected on 09.21 by the stricter Stage 2 rule were still in the chart, and 4 mine-tailings articles were rejected today at Stage 3 (regionalization) — 5 more pairs.<br><b>1,208 → 1,169</b> pairs.'
      },
    ...
    ZH: {
      ...
      funnelLabel: '筛选',
      funnelTitle: '筛选漏斗：从5,093条记录到531篇文献',
      ...
      funnelDetails: {
        universo: '检索共收集5,093条记录。其中2,627条（51.6%）为重复记录，在任何筛选之前已被移除。1,869条在随后四个阶段中被剔除——详见下方各阶段柱状图。7条从未能获取全文（无法访问）。剩余590条中，588条——531篇入选文献+57篇无自身生命周期评估的综述——进入第4阶段；另有2篇在一次专项复核中入选的文献尚待第3、4阶段评估。',
        e1: '在按元数据（年份、文献类型、可模塑材料、建筑用途）筛选的2,471条记录中，2,199条被接受。被排除的272条中：190条因不属于建筑用途，71条因材料不可模塑，9条因文献类型，1条因发表年份——同一记录可能同时不符合多项标准。',
        e2: '在通读全文PDF的2,188篇文献中，605篇被接受。被排除的1,583篇中：1,297篇未报告可与自身CO₂配对的自身抗压强度，388篇未报告有绝对数值的CO₂/生命周期评估结果——部分文献两项标准都未满足。',
        e3: '在评估材料地域限制的599篇文献中，588篇被接受；11篇因主要材料受地域限制（例如某地区高度集中的生产副产品）而被排除。',
        e4: '在第4阶段（7项清单，0至7分）评估的588篇文献中：422篇构成主要结果（得分≥4.0），109篇仅用于敏感性分析（得分<4.0），57篇是没有自身生命周期评估的综述——不计入ci指标得分，但作为PRISMA引文分支的来源（这些综述引用的原始研究）。'
      },
      tierLabel: '质量分层',
      ...
      progressCorpoProvaP: '已提取试件（或被测构件）体积的数据行计数——为将来的立方体转圆柱体换算做准备。该项追踪始于2026年9月12日；专项提取（437篇文献，30批）已于2026年9月24日完成。',
      progressIcTitle: '各研究的置信区间',
      progressIcP: '已从原文提取标准差、变异系数或置信区间的数据行计数——即第4阶段QG标准的依据（见下方"关于该指标"）。该列于2026年9月12日创建；第4阶段评估的所有文献均已完成QG评分。',
      progressWave: '第',
      ...
      progressMarcos: {
        ...
        marcoRejeitados: '<b>修正 · 2026.09.26</b><br>仪表板现在只包含入选文献。此前，9月21日按更严格的第2阶段规则剔除的6篇文献仍有34对数据留在图中；另有4篇矿山尾矿文献今天在第3阶段（地域性）被剔除——再减少5对。<br><b>1,208 → 1,169</b> 对数据。'
      },
      scatterTitle: '强度 × 碳足迹',
  ...
  var KPI_VALUES = {
    ...
    /* % de artigos Elegível com pelo menos um par fck+CO2 completo em
       ...
       Snapshot 2026-09-26 (fim dos 14 achados CI_MPA): 537 Elegível, 2
       isentas, 458 dos 535 com par completo = 86%. funil = 535 Elegível que
       passaram as 4 etapas (3001 e 3163, revisões aceites a 2026-09-14,
       ainda sem E3/E4; 2687 e 4931 pontuados a 2026-09-26). vida = artigos Elegível com vida_util_anos em
       dados_fck.
